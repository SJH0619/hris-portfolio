...
create table employee(
    ...
    insert_at date not null comment '생성일',
...
create table department(
    ...
    foundation_date date not null comment '설립일',
    insert_at date not null comment '생성일',
    insert_by char(4) not null comment '생성자',
    update_at date comment '수정일',
    update_by char(4) comment '수정자'
...
create table performance_evaluation(
    ...
    performance_evaluation_start_date date not null comment '성과 평가 시작 날짜',
    performance_evaluation_end_date date comment '성과 평가 종료 날짜',
...
create table vacation(
    ...
    vacation_request_date date comment '휴가 신청 날짜',
    vacation_approval_date date comment '휴가 승인 날짜',
    insert_at date not null comment '생성일',
    insert_by char(4) not null comment '생성자',
    update_at date comment '수정일',
    update_by char(4) comment '수정자',
...
create table attendance(
    ueid char(4) comment '직원 고유 식별자',
    attendance_date date comment '날짜',
    attendance_day char(1) not null comment '요일',
    arrival_time date not null comment '출근 시각',
    departure_time date not null comment '퇴근 시각',
